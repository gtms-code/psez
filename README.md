...
- **文字列検索・置換** — `Ctrl+F` で検索、`Ctrl+R` で全置換が可能です
- **長い行の表示** — 画面幅を超える行は横スクロール、または折り返し表示（Ctrl+B で切り替え）に対応します
...
Windows 10 / 11 には [winget](https://learn.microsoft.com/ja-jp/windows/package-manager/winget/) が標準搭載されています。以下のコマンド一発でインストールできます。

```sh
winget install gtms-code.psez
```
...
`Ctrl+F` を押すと画面下部に検索プロンプトが開きます。

- 文字を入力すると一致箇所がハイライト表示され、最初のヒットにカーソルが移動します
- `Enter` を押すたびに次のヒットへ移動します（末尾から先頭に折り返します）
- `Esc` で検索を閉じます

### 文字列の全置換（Ctrl+R）

`Ctrl+R` を押すと2ステップで置換できます。

1. 検索文字列を入力して `Enter`
2. 置換後の文字列を入力して `Enter` → 一致するすべての箇所を一括置換します

置換は `Ctrl+Z` でアンドゥできます。`Esc` でいつでもキャンセルできます。

### 折り返し表示（Ctrl+B）

`Ctrl+B` を押すたびに折り返し表示のON / OFFを切り替えます。
